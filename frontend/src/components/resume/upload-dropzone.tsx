...
export function UploadDropzone({ onUploaded, compact = false }: UploadDropzoneProps) {
  ...
  return (
    <div className="space-y-3">
      {!file && (
        <div
          ...
          className={cn(
            ...
            compact ? "p-6" : "p-10",
            isDragActive
              ? "border-accent-v2 bg-accent-soft"
              : "border-border bg-surface-2 hover:border-accent-v2/40 hover:bg-accent-soft/40",
            isDragReject && "border-danger bg-[#F8E3E0]"
          )}
        >
          <input {...getInputProps()} />
      ...
      {file && (
        <div className="rounded-3xl border border-border bg-surface p-4 flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-accent-soft flex items-center justify-center text-accent-strong shrink-0">
            <FileText size={16} />
          </div>
