...
import { useDiff } from "@/hooks/use-resumes";

function GradientNumber({ value, size = 32, palette = "sage" }: any) {
  const gradient =
    palette === "danger"
...
function VersionPicker({ versions, value, onChange, exclude }: any) {
  return (
    <div className="inline-flex items-center gap-0.5 bg-surface border border-border p-0.5 rounded-full shadow-card">
      {versions.map((v: any) => {
...
function ModeToggle({ mode, onChange }: any) {
  return (
    <div className="inline-flex items-center gap-0.5 bg-surface-2 border border-border p-0.5 rounded-full">
      {["words", "lines"].map((m) => (
        <button
          key={m}
          onClick={() => onChange(m)}
...
export function DiffView({ resumeId, versions }: { resumeId: string; versions: any[] }) {
  ...
  const { data, isLoading, error } = useDiff(resumeId, fromId, toId, mode as any);
  ...
  return (
    <Card>
      ...
      {/* Compare bar */}
      <div className="rounded-2xl border border-border bg-surface-2/60 p-3 mb-5 flex items-center justify-center gap-3 flex-wrap">
      ...
      {error && (
        <div className="text-xs text-danger bg-[#F8E3E0] rounded-xl px-3 py-2">
      ...
      {data && (
        <>
          ...
          >
            ...
            <div className="relative flex items-end gap-8 flex-wrap">
              ...
              <div className="ml-auto text-right">
                ...
                <div className="mt-1.5 inline-flex items-center gap-1.5 text-[12px] font-semibold text-ink tabular-nums">
                  {fromLabel}
                  <ArrowRight
                    size={11}
          ...
          <div className="rounded-2xl border border-border bg-surface overflow-hidden">
            <div className="h-10 px-4 flex items-center justify-between border-b border-border bg-surface-2/60">
              <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-semibold text-ink-muted">
                <FileText size={11} />
                Inline diff · {mode}
              </div>
              <div className="flex items-center gap-3 text-[10px] text-ink-muted">
                <span className="inline-flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-sm bg-accent-soft border border-accent-v2/30" />
                  Added
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-sm bg-[#F8E3E0] border border-danger/30" />
                  Removed
                </span>
              </div>
            </div>
            <div className="p-5 max-h-[600px] overflow-auto font-mono text-[13px] leading-[1.75] whitespace-pre-wrap text-ink-muted">
              {data.parts.map((p: any, i: number) => (
                <span
                  key={i}
                  className={cn(
                    !p.added && !p.removed && "text-ink",
                    p.added &&
                      "bg-accent-soft text-accent-strong font-medium rounded px-0.5 py-0.5",
                    p.removed &&
                      "bg-[#F8E3E0] text-danger line-through decoration-danger/50 rounded px-0.5 py-0.5"
                  )}
                >
                  {p.value}
                </span>
