...
function Chip({ children, tone }: { children: React.ReactNode; tone: "present" | "missing" }) {
  if (tone === "present") {
    return (
      <span className="inline-flex items-center gap-1.5 h-7 pl-1.5 pr-3 rounded-full bg-accent-soft text-accent-strong text-[12px] font-semibold transition-transform hover:-translate-y-px">
        <span className="h-4 w-4 rounded-full bg-accent-strong text-white flex items-center justify-center">
  ...
  return (
    <span className="inline-flex items-center gap-1.5 h-7 pl-1.5 pr-3 rounded-full bg-[#F8E3E0]/70 text-danger text-[12px] font-semibold transition-transform hover:-translate-y-px">
      <span className="h-4 w-4 rounded-full bg-danger text-white flex items-center justify-center">
...
function SectionHeader({ tone, label, count }: { tone: "present" | "missing"; label: string; count: number }) {
  ...
  return (
    <div className="flex items-center gap-2.5 mb-3">
      <span
        className={
          isPresent
            ? "h-6 w-6 rounded-lg bg-accent-soft text-accent-strong flex items-center justify-center"
            : "h-6 w-6 rounded-lg bg-[#F8E3E0] text-danger flex items-center justify-center"
        }
      >
...
export function KeywordChips({ present = [], missing = [] }: { present?: string[]; missing?: string[] }) {
  ...
  return (
    <Card>
      ...
      >
        ...
        <div className="relative flex items-end justify-between gap-4">
          <div>
            ...
            <div className="flex items-baseline gap-1.5 mt-1.5">
              ...
              <span className="text-ink-muted text-base font-medium tabular-nums">
                / {total}
              </span>
        ...
        <div className="relative mt-4 h-1.5 w-full rounded-full bg-surface/60 overflow-hidden">
          <div
      ...
      <div className="space-y-5">
        <div>
          ...
          {present.length ? (
            ...
            <p className="text-xs text-ink-muted pl-8">None detected.</p>
          )}
        </div>
        ...
        <div>
          ...
          {missing.length ? (
            ...
            <p className="text-xs text-ink-muted pl-8">
