    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={handleAnalyze} disabled={!selectedDocId || isAnalyzing}>
      ...
      {!result && !isAnalyzing && (
        <Card className="border-dashed border-border/50 bg-card/20 flex flex-col items-center justify-center py-24 text-center">
      ...
      {isAnalyzing && (
        <Card className="border-border/50 bg-card/50 flex flex-col items-center justify-center py-24 text-center">
      ...
      {result && !isAnalyzing && (
        <div className="grid gap-6 md:grid-cols-3">
          {/* Main Score Card */}
          <Card className="md:col-span-1 border-border/50 bg-card/50 backdrop-blur-sm flex flex-col justify-center items-center py-8">
            <div className="relative flex items-center justify-center h-40 w-40">
              {/* Simple Circular Progress Placeholder using SVG */}
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle cx="80" cy="80" r="70" stroke="currentColor" strokeWidth="12" fill="transparent" 
                  strokeDasharray="440" strokeDashoffset={440 - (440 * result.atsScore) / 100}
                  className={`${result.atsScore >= 80 ? 'text-success' : result.atsScore >= 60 ? 'text-warning' : 'text-danger'} transition-all duration-1000 ease-in-out`} />
              </svg>
                  ...
                  <div className="rounded-lg border border-border/50 bg-muted/20 p-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {result.presentKeywords.length === 0 ? <span className="text-xs text-muted-foreground">None</span> : 
                        result.presentKeywords.map(kw => <span key={kw} className="text-xs bg-success/10 text-success px-2 py-0.5 rounded-full">{kw}</span>)
                      }
                    </div>
                  </div>
                  <div className="rounded-lg border border-border/50 bg-muted/20 p-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {result.missingKeywords.length === 0 ? <span className="text-xs text-muted-foreground">None</span> : 
                        result.missingKeywords.map(kw => <span key={kw} className="text-xs bg-danger/10 text-danger px-2 py-0.5 rounded-full">{kw}</span>)
                      }
                    </div>
                  </div>
                  <div className="rounded-lg border border-border/50 bg-muted/20 p-4 col-span-2">
                    <div className="flex items-center gap-2 mb-2">
                      <AlertCircle className="h-4 w-4 text-warning" />
                      <h4 className="text-sm font-medium">Formatting Issues</h4>
                    </div>
              ...
              <CardContent>
                <ul className="space-y-4">
                  {result.improvements.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Your resume is looking great! No major improvements needed.</p>
                  ) : (
                    result.improvements.map((imp, i) => (
                      ...
                      >
                        ...
                        <div className="pl-6 grid gap-2">
                          <div className="text-muted-foreground bg-danger/5 border border-danger/10 p-2 rounded line-through">
                            {imp.current}
                          </div>
                          <div className="text-foreground bg-success/5 border border-success/10 p-2 rounded">
                            {imp.suggested}
                          </div>
