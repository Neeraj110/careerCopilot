...
export default function Insights() {
    ...
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          ...
          action={
            <Button variant="accent" size="md" onClick={() => router.push("/dashboard/resumes")}>
              Go to resumes
            </Button>
          }
        />
  ...
  const trend = (data.scoreTrend || []).map((p: any, i: number) => ({
    label: `#${i + 1}`,
    score: p.score,
    at: p.at,
    ...
    <div className="space-y-6">
              ...
              <Tooltip
                content={({ active, payload }) =>
                  active && payload?.length ? (
                    <div className="rounded-xl bg-surface border border-border shadow-hover px-3 py-2 text-xs">
            ...
            <div className="space-y-3">
              {data.topIssues.map((issue: any, i: number) => (
                <div key={i} className="flex items-start gap-3">
                  <div className="h-8 w-8 rounded-xl bg-surface-2 flex items-center justify-center text-ink-muted shrink-0">
                    <AlertCircle size={14} />
                  </div>
        ...
        <Card>
          ...
          {data.topMissingKeywords.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {data.topMissingKeywords.map((k: any, i: number) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-[#F8E3E0] text-danger"
                >
                  {k.keyword}
      ...
      {data.topPresentKeywords?.length > 0 && (
        <Card>
          ...
          <div className="flex flex-wrap gap-2">
            {data.topPresentKeywords.map((k: any, i: number) => (
              <span
                key={i}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-accent-soft text-accent-strong"
              >
                {k.keyword}
                <span className="tabular text-[10px] opacity-70">×{k.count}</span>
              </span>
      ...
      <Card>
        ...
        <div className="overflow-x-auto -mx-2">
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.resumePerformance.map((r: any) => (
                <tr
                  key={r.resumeId}
                  onClick={() => router.push(`/dashboard/resumes/${r.resumeId}`)}
...
function Kpi({ label, value, suffix, sub, icon: Icon, accent }: { label: string; value: string | number; suffix?: string; sub?: string; icon: any; accent?: boolean }) {
  return (
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1.5 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs ${accent ? "text-white/70" : "text-ink-muted"}`}
            >
              {label}
            </span>
          </div>
          <div className="flex items-baseline gap-1">
            ...
            {suffix && (
              <span
                className={`text-sm ${accent ? "text-white/70" : "text-ink-muted"}`}
              >
                {suffix}
              </span>
            )}
          ...
          {sub && (
            <div
              className={`text-xs truncate ${accent ? "text-white/80" : "text-ink-muted"}`}
            >
              {sub}
            </div>
          )}
